    ...
    <Container>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        {currentUser ? (
          ...
          <OverlayTrigger
            placement="top"
            overlay={<Tooltip id="tooltip-top">You must login to create a review!</Tooltip>}
          >
            <div className={`d-flex align-items-center ${styles.reviewcreate}`} style={{ cursor: 'not-allowed' }}>
              <i className="fa-solid fa-pencil" style={{ color: 'red' }}></i>
              <span className={styles.writeReviewText}>Write a Review</span>
            </div>
          </OverlayTrigger>
      ...
      <ListGroup>
        {reviews.map((review) => (
          <Link key={review.id} to={`/reviews/${review.id}`} className={`${styles.reviewLink}`}>
